...
const Calendar: React.FC = () => {
  ...
  React.useEffect(() => {
    console.log("Selecte time", selectedTime)
    setBookingData({
      ...bookingData,
      slot: typeof(selectedTime) === typeof(2) ? availableSlots[selectedTime] : undefined,
    })
    localStorage.setItem('booking-details', JSON.stringify({
      ...bookingData,
      slot: typeof(selectedTime) === typeof(2) ? availableSlots[selectedTime] : undefined,
  ...
  return (
    ...
    {(!selectedTime || (selectedTime && !isPaymentStep)) &&  <div className="calendar-container">
       ...
       {!isPaymentStep && <button onClick={handleTimeSelectionEnable} className={`mt-4 flex button button-dark gap-1 items-center justify-center ${!selectedDate && 'disable'}`}>
